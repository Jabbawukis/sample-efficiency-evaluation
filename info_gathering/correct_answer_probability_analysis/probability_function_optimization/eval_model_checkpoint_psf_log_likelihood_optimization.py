...
def power_scaling_function(alpha, x):
    return 0 if x == 0 else 1 - np.power(1 / x, alpha)
    ...
    if optimize_over_all_slices:
        for bear_size in bear_sizes:
            ...
            for model in models:
                ...
                optimized_alphas.append(
                    {"Model": model, "Alpha": optimize_alphas(slice_data, vectorized_psf, optimize_over_all_slices)}
                )
            ...
            for model in optimized_alphas:
                ...
                save_dict_as_json(
                    model,
                    f"{_output_path}/psf_optimized_alpha_over_all_slices.json",
                )
        ...
        for bear_size in bear_sizes:
            ...
            for model in models:
                ...
                optimized_alphas.append({"Model": model, "Alphas": optimize_alphas(slice_data, vectorized_psf)})
            ...
            output_path_diagram = f"{abs_path}/sample_efficiency_evaluation_results/correct_answer_probability_analysis_plots/BEAR-{bear_size}/power_scaling_function/"

            if not os.path.exists(output_path_diagram):
                os.makedirs(output_path_diagram)

            plot_alphas(
                optimized_alphas, output_path_diagram, output_diagram_name=f"psf_optimized_alphas_bear_{bear_size}"
            )
